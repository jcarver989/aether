use async_openai::types::chat::{
    ...
    ChatCompletionTool, ChatCompletionTools, FunctionCall, FunctionObject, ImageUrl, InputAudio,
    InputAudioFormat,
};
...
fn map_message(msg: ChatMessage) -> Result<Option<ChatCompletionRequestMessage>> {
    match msg {
        ChatMessage::System { content, .. } => Ok(Some(ChatCompletionRequestMessage::System(
            ChatCompletionRequestSystemMessage {
                content: content.into(),
                name: None,
            },
        ))),
        ChatMessage::User { content, .. } => Ok(Some(ChatCompletionRequestMessage::User(
        ...
        ChatMessage::Assistant {
            ...
            Ok(Some(ChatCompletionRequestMessage::Assistant(
        ...
        ChatMessage::ToolCallResult(result) => {
            ...
            Ok(Some(ChatCompletionRequestMessage::Tool(
        ...
        ChatMessage::Summary { content, .. } => Ok(Some(ChatCompletionRequestMessage::User(
            ChatCompletionRequestUserMessage {
...
fn map_user_content(parts: Vec<ContentBlock>) -> Result<ChatCompletionRequestUserMessageContent> {
    let openai_parts: Vec<ChatCompletionRequestUserMessageContentPart> = parts
        .into_iter()
        .map(map_user_content_part)
        .collect::<Result<_>>()?;

    Ok(ChatCompletionRequestUserMessageContent::Array(openai_parts))
}

fn map_user_content_part(
    part: ContentBlock,
) -> Result<ChatCompletionRequestUserMessageContentPart> {
    match part {
        ContentBlock::Text { text } => Ok(ChatCompletionRequestUserMessageContentPart::Text(
        ...
        ContentBlock::Audio { data, mime_type } => {
            ...
            Ok(ChatCompletionRequestUserMessageContentPart::InputAudio(
