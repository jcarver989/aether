            ...
            StreamChunk::ToolCallComplete { id } => {
                if let Some(partial) = agent.active_tool_calls_mut().remove(&id) {
                    ...
                    if let Ok(args) = serde_json::from_str::<serde_json::Value>(&arguments_str) {
                        agent.record_tool_call(partial.name, args, SystemTime::now());
                    }
                }
            }
            StreamChunk::Done => {
...
async fn test_agent_tool_execution_with_real_mcp() {
    use aether_core::testing::full_integration::{FileServerMcp, connect};
    ...
    let llm_provider = FakeLlmProvider::with_single_response(vec![
        StreamChunk::Content("I'll write the file for you.".to_string()),
        StreamChunk::Done,
    ]);
    let mut agent = Agent::new(llm_provider, tool_registry, None);
    ...
    assert_eq!(
        agent.get_server_for_tool("write_file"),
        Some(&"file_server".to_string())
    );
    ...
    let tool_call_result = mcp_client
...
async fn test_agent_execute_tool_end_to_end() {
    ...
    let tools_response = mcp_service
        .list_tools(None)
        .await
        .expect("Failed to list tools from MCP server");
    ...
    // Note: This is a limitation in the current design - the aether_coremcp::McpClient
    ...
    let direct_result = mcp_service
        ...
        .await
        .expect("Direct tool execution should work");

    assert_eq!(direct_result.is_error, Some(false));

    // Verify the file was written
    let file_content = filesystem
        .read_file("/tmp/direct_test.txt")
        .await
...
async fn test_agent_system_prompt() {
    let llm_provider = FakeLlmProvider::with_single_response(vec![
        StreamChunk::Content("Test response".to_string()),
        StreamChunk::Done,
    ]);
    let tool_registry = create_test_tool_registry();
    ...
    if let aether_core::llm::ChatMessage::System { content } = &llm_messages[0] {
