...
async fn test_cancel_message_variant() {
    ...
    let has_cancelled_message = events.iter().any(|e| {
        matches!(e, AgentMessage::Cancelled { .. })
    });
...
async fn test_cancellation_during_tool_execution() {
    ...
    while let Some(event) = stream.next().await {
        ...
        if matches!(event, AgentMessage::ToolCall { is_complete: false, .. }) {
            tool_started = true;
    ...
    let has_cancelled_event = events.iter().any(|e| {
        matches!(e, AgentMessage::Cancelled { .. })
    });
...
async fn test_multiple_operations_with_cancellation() {
    ...
    assert!(cancel_token1.is_cancelled(), "First token should be cancelled");
    assert!(!cancel_token2.is_cancelled(), "Second token should not be cancelled");

    // Second operation should complete normally with some text
    let has_second_text = second_events.iter().any(|e| {
        matches!(e, AgentMessage::Text { chunk, .. } if !chunk.is_empty())
    });

    // Check that we got a complete message
    let has_complete_message = second_events.iter().any(|e| {
        matches!(e, AgentMessage::Text { is_complete: true, .. })
    });

    assert!(has_second_text, "Second operation should have some text output");
    assert!(has_complete_message, "Second operation should complete normally");
}
