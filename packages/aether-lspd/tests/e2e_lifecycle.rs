...
async fn poll_workspace_diagnostics(
    ...
    while start.elapsed() < timeout {
        let diagnostics = client
            ...
            .await
            .expect("Failed to get workspace diagnostics");
...
async fn daemon_persists_after_client_disconnect() {
    ...
    let client = LspClient::connect(project.root(), LanguageId::Rust)
        .await
    ...
    assert!(
        ...
        "Daemon should continue running after a client disconnects"
    );
}
...
async fn multiple_clients_share_fake_server_session() {
    ...
    let hover1 = hover_text(
        client1
            .hover(uri.clone(), 0, 0)
            .await
            .expect("Hover failed"),
    );
    let hover2 = hover_text(
        client2
            .hover(uri.clone(), 0, 0)
            .await
            .expect("Hover failed"),
...
async fn workspace_diagnostics_refresh_after_external_edit_without_explicit_open() {
    ...
    let project =
        CargoProject::new("workspace_external_refresh").expect("Failed to create project");
    project
        .add_file("src/main.rs", "fn main() { let ok = 1; }\n")
        .expect("Failed to add source file");
