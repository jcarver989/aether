...
"#;
    ...
    let definition = client
        .goto_definition(uri.clone(), 4, 16)
        .await
    ...
    let implementation = client
        .goto_implementation(uri.clone(), 4, 16)
        .await
        .expect("Goto implementation failed");
    ...
    let references = client
        .find_references(uri.clone(), 0, 3, true)
        .await
    ...
    let workspace_symbols = client
        .workspace_symbol("example".to_string())
        .await
        .expect("Workspace symbol failed");
    assert!(
        workspace_symbols
            .iter()
            .any(|symbol| symbol.name == "example_fn")
    );

    let call_items = client
        .prepare_call_hierarchy(uri.clone(), 4, 16)
        .await
    ...
    let incoming = client
        .incoming_calls(call_items[0].clone())
        .await
    ...
    let outgoing = client
        .outgoing_calls(call_items[0].clone())
        .await
        .expect("Outgoing calls failed");
    ...
    let edits = changes
        .get(&uri)
        .expect("Rename changes should include the file");
...
async fn diagnostic_helpers_round_trip_through_fake_server() {
    ...
    project
        .add_file("src/main.rs", "fn main() { let error = 1; }\n")
        .expect("Failed to add file");
    ...
    client
        .queue_diagnostic_refresh(uri.clone())
        .await
        .expect("Queue diagnostic refresh failed");
