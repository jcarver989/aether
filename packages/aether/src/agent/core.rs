...
impl<T: StreamingModelProvider + 'static> Agent<T> {
    ...
    fn inject_continuation_prompt(&mut self, previous_response: &str) {
        ...
        self.context.add_message(ChatMessage::User {
            content: format!("<system-notification>The LLM API stopped without a '{}' signal. If your task is complete, ensure your final response ends with {}. If you're still working, continue from where you left off.</system-notification>", COMPLETION_SIGNAL, COMPLETION_SIGNAL),
            timestamp: IsoString::now(),
        });
    }
