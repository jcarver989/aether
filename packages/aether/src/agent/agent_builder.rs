...
pub struct SpawnedAgent {
    ...
    pub rx: mpsc::UnboundedReceiver<AgentMessage>,
}
...
impl<T: ModelProvider + 'static> AgentBuilder<T> {
    ...
    pub async fn spawn(self) -> Result<SpawnedAgent> {
        let (user_message_tx, mut user_message_rx) = mpsc::unbounded_channel::<UserMessage>();
        let (agent_message_tx, agent_message_rx) = mpsc::unbounded_channel::<AgentMessage>();
        ...
        let task_handle = tokio::spawn(async move {
            while let Some(user_message) = user_message_rx.recv().await {
                ...
                while let Some(agent_message) = response_stream.next().await {
                    if agent_message_tx.send(agent_message).is_err() {
