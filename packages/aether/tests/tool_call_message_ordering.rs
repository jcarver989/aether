...
#[tokio::test]
async fn test_multiple_fast_tool_calls_race_condition() {
    // Test with multiple tool calls that complete very quickly
    // This increases the likelihood of hitting the race condition
    ...
    let responses = vec![
        ...
        LlmResponse::Text {
            chunk: "I'll use multiple tools".to_string(),
        },
        // First tool call
        LlmResponse::ToolRequestStart {
            id: "call_1".to_string(),
        ...
        LlmResponse::ToolRequestArg {
            id: "call_1".to_string(),
            chunk: r#"{"value": "first"}"#.to_string(),
        },
        LlmResponse::ToolRequestComplete {
            tool_call: ToolCallRequest {
                id: "call_1".to_string(),
                name: "fast_tool".to_string(),
                arguments: r#"{"value": "first"}"#.to_string(),
            },
        },
        // Second tool call
        LlmResponse::ToolRequestStart {
        ...
        LlmResponse::ToolRequestArg {
            id: "call_2".to_string(),
            chunk: r#"{"value": "second"}"#.to_string(),
        },
        LlmResponse::ToolRequestComplete {
            tool_call: ToolCallRequest {
                id: "call_2".to_string(),
                name: "fast_tool".to_string(),
                arguments: r#"{"value": "second"}"#.to_string(),
            },
        },
        LlmResponse::Done,
    ];
    ...
    let mut receiver = agent.send(UserMessage::text("Use multiple fast tools")).await;
    ...
    assert_eq!(tool_results.len(), 2, "Should have received 2 tool results");
    println!("✅ Multiple fast tool calls test passed!");
}