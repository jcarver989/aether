...
| `plan` | [`PlanMcp`](src/plan/README.md) | Submit markdown plans for approval via external reviewer command or native elicitation |
...
```json
{
  "servers": {
    ...
    "plan": {
      "type": "in-memory",
      "args": ["--command", "./scripts/plannotator-mcp-adapter"]
    }
  }
}
```
...
| `plan` | `--command <value>` | none | Optional external reviewer command executed via `bash -lc`; command stdout must be JSON `{ approved: bool, feedback?: string }` |
...
let builder = mcp()
    ...
    .register_in_memory_server("plan", Box::new(|args| {
