...
async fn run_session_relay(
    ...
    if let Err(e) = connection.send_mcp_notification(McpNotification::ServerStatus { servers: initial_server_statuses })
    {
...
struct PromptContext<'a> {
    ...
    connection: &'a dyn AcpConnection,
...
async fn handle_elicitation_request(connection: &dyn AcpConnection, elicitation: ElicitationRequest) {
    let params = build_elicitation_params(&elicitation.server_name, &elicitation.request);

    let mcp_result = match connection.request_elicitation(params).await {
        Ok(response) => {
...
async fn authenticate_mcp_server(
    ...
    if let Err(e) = connection.send_mcp_notification(McpNotification::ServerStatus { servers: statuses }) {
...
fn forward_notification(connection: &dyn AcpConnection, acp_session_id: &SessionId, msg: &AgentMessage) {
    ...
    if let AgentMessage::ToolResult { result_meta, .. } = msg
        ...
        && let Err(e) = connection.send_session_notification(plan_notif)
    {
...
) -> Result<(), acp_utils::server::AcpServerError> {
    use super::mappers::AgentExtNotification;
    match notification {
        AgentExtNotification::ContextUsage(p) => connection.send_context_usage(p),
        AgentExtNotification::ContextCleared(p) => connection.send_context_cleared(p),
        AgentExtNotification::SubAgentProgress(p) => connection.send_sub_agent_progress(p),
    }
}

async fn handle_mcp_client_event(connection: &dyn AcpConnection, event: McpClientEvent) {
    match event {
        ...
        McpClientEvent::UrlElicitationComplete(params) => {
            if let Err(e) = connection.send_mcp_notification(McpNotification::UrlElicitationComplete(params)) {
...
mod tests {
    ...
    async fn url_elicitation_complete_is_forwarded_as_mcp_notification() {
        ...
        let sent = connection.mcp_notifications();
        assert_eq!(sent.len(), 1);
        assert!(matches!(sent[0], McpNotification::UrlElicitationComplete(_)));
    }

    #[tokio::test]
    async fn elicitation_request_forwards_response_from_peer() {
        let connection = FakeAcpConnection::new();
        connection.queue_elicitation_response(Ok(acp_utils::notifications::ElicitationResponse {
        ...
        let calls = connection.elicitation_requests();
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].server_name, "test-server");
    }

    #[tokio::test]
    async fn elicitation_request_surfaces_cancel_on_transport_error() {
        let connection = FakeAcpConnection::new();
        // queue_elicitation_response not called → default ConnectionUnavailable is returned.
