//! Shared types for ACP protocol extensions.
//!
//! This crate provides types used by both the ACP server (`aether-acp`) and
//! client (`aether-acp-client`) for custom protocol extensions.
...
///
/// This type is used for serialization/deserialization of the `_aether/context_usage`
/// custom notification payload on both the agent (server) and client sides.
