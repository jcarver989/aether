...
/// A higher-level focus manager that combines [`FocusRing`] with Tab/BackTab handling.
///
/// `FocusGroup` wraps a `FocusRing` and adds UiEvent-based navigation. Use
/// [`navigation`](Self::navigation) to handle Tab/BackTab in [`InteractiveComponent::on_event`].
///
/// # Example
///
/// ```rust
/// use tui::{Component, FocusGroup, InteractiveComponent, Line, MessageResult, NavigationResult, RenderContext, UiEvent};
///
/// struct MyForm {
///     focus: FocusGroup,
/// }
...
/// }
///
/// impl InteractiveComponent for MyForm {
///     type Message = ();
///
///     fn on_event(&mut self, event: UiEvent) -> MessageResult<Self::Message> {
///         match self.focus.navigation(&event) {
///             NavigationResult::Moved => return MessageResult::consumed(),
///             NavigationResult::Ignored => {}
///         }
///
///         // Route to focused child based on self.focus.focused()
...
impl FocusGroup {
    /// Create a new `FocusGroup` with the given number of focusable items.
    pub fn new(len: usize) -> Self {
        Self {
            ring: FocusRing::new(len),
        }
    }
    ...
    /// Returns `true` if `index` is the currently focused index.
    pub fn is_focused(&self, index: usize) -> bool {
        self.ring.is_focused(index)
    }

    /// Update the number of focusable items.
    pub fn set_len(&mut self, len: usize) {
        self.ring.set_len(len);
    }

    /// Set focus to a specific index. Returns `false` if out of bounds.
    pub fn focus(&mut self, index: usize) -> bool {
        self.ring.focus(index)
    }

    /// Handle Tab/BackTab navigation.
    ///
    /// Returns [`NavigationResult::Moved`] if focus changed, or
    /// [`NavigationResult::Ignored`] for non-navigation keys.
    pub fn navigation(&mut self, event: &UiEvent) -> NavigationResult {
        match event {
            UiEvent::Key(key) => match self.ring.handle_key(*key) {
                FocusOutcome::FocusChanged => NavigationResult::Moved,
                FocusOutcome::Unchanged | FocusOutcome::Ignored => NavigationResult::Ignored,
            },
            _ => NavigationResult::Ignored,
        }
    }
}

/// Result of [`FocusGroup::navigation`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum NavigationResult {
    /// Focus moved to a different index.
    Moved,
    /// The event was not a navigation key, or focus didn't change.
    Ignored,
}
...
mod focus_group_tests {
    ...
    #[test]
    fn new_creates_group_with_focus_ring() {
        let group = FocusGroup::new(3);
        assert_eq!(group.focused(), 0);
        assert!(group.is_focused(0));
    }

    #[test]
    fn navigation_tab_moves_forward() {
        let mut group = FocusGroup::new(3);
        let result = group.navigation(&UiEvent::Key(key(KeyCode::Tab)));
        assert_eq!(result, NavigationResult::Moved);
        assert_eq!(group.focused(), 1);
    }

    #[test]
    fn navigation_backtab_moves_backward() {
        let mut group = FocusGroup::new(3);
        group.focus(1);
        let result = group.navigation(&UiEvent::Key(key(KeyCode::BackTab)));
        assert_eq!(result, NavigationResult::Moved);
        assert_eq!(group.focused(), 0);
    }

    #[test]
    fn navigation_ignores_other_keys() {
        let mut group = FocusGroup::new(3);
        let result = group.navigation(&UiEvent::Key(key(KeyCode::Enter)));
        assert_eq!(result, NavigationResult::Ignored);
        assert_eq!(group.focused(), 0);
    }
    ...
    #[test]
    fn focus_and_set_len_delegate_to_ring() {
        let mut group = FocusGroup::new(5);
        assert!(group.focus(3));
        assert_eq!(group.focused(), 3);

        group.set_len(2);
        assert_eq!(group.focused(), 1); // clamped
    }
}
