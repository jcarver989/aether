...
fn integer_input_renders() {
    ...
    nf.on_event(UiEvent::Key(key(KeyCode::Char('-'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('4'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('2'))));
    let term = render_component(&nf, 80, 24);
...
fn float_input_renders() {
    ...
    nf.on_event(UiEvent::Key(key(KeyCode::Char('3'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('.'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('1'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('4'))));
    let term = render_component(&nf, 80, 24);
...
fn rejects_alpha() {
    let mut nf = NumberField::new(String::new(), false);
    nf.on_event(UiEvent::Key(key(KeyCode::Char('1'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('a'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('2'))));
    let term = render_component(&nf, 80, 24);
    assert_buffer_eq(&term, &["12▏"]);
}
...
fn backspace_renders() {
    let mut nf = NumberField::new(String::new(), true);
    nf.on_event(UiEvent::Key(key(KeyCode::Char('9'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Char('9'))));
    nf.on_event(UiEvent::Key(key(KeyCode::Backspace)));
    let term = render_component(&nf, 80, 24);
