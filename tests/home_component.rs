...
mod tests {
    ...
    fn test_home_forwards_input_key_events() {
        ...
        assert!(action.is_none(), "Home should not return action for char input");
    ...
    fn test_home_forwards_enter_key_to_submit_message() {
        ...
        for ch in chars.chars() {
            home.handle_key_event(KeyEvent::new(KeyCode::Char(ch), KeyModifiers::NONE)).unwrap();
        }
    ...
    fn test_home_input_and_chat_dont_interfere() {
        ...
        let chat_line_idx = lines.iter().position(|line| line.contains("This is a chat message"));
        let input_line_idx = lines.iter().position(|line| line.contains("user input text"));
        
        assert!(chat_line_idx.is_some(), "Chat message should be found in buffer");
        assert!(input_line_idx.is_some(), "Input text should be found in buffer");
        assert!(
    ...
    fn test_home_updates_chat_with_new_messages() {
        ...
        let updated_content = extract_buffer_text(&updated_buffer, 0, updated_buffer.content().len());
    ...
    fn test_home_handles_tool_call_display() {
        ...
        home.update(Action::AddChatMessage(tool_result_msg)).unwrap();
    ...
    fn test_home_input_clears_after_submit() {
        ...
        assert!(before_content.contains("test message"), "Message should be in input before submit");
        
        // Submit message
        home.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)).unwrap();
    ...
    fn test_home_handles_complex_input_editing() {
        ...
        for _ in 0..6 {  // Move back to position after "Hello"
            home.handle_key_event(KeyEvent::new(KeyCode::Left, KeyModifiers::NONE)).unwrap();
        }
    ...
    fn test_home_preserves_chat_scroll_position() {
        ...
        // Scroll up
        home.update(Action::ScrollChat(ScrollDirection::Up)).unwrap();
        home.update(Action::ScrollChat(ScrollDirection::Up)).unwrap();
    ...
    fn test_home_handles_escape_clear_input() {
        ...
        let before_buffer = draw_home_component(&mut home, TEST_BUFFER_WIDTH, TEST_BUFFER_HEIGHT);
        let before_content = extract_buffer_text(&before_buffer, 0, before_buffer.content().len());
        assert!(before_content.contains("text to clear"), "Text should be present before clear");
