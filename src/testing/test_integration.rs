use crate::testing::{create_transport_pair, InMemoryFileSystem};
...
mod tests {
    ...
    async fn test_in_memory_filesystem_integration() {
        ...
        // Test writing a file
        filesystem.write_file("/test/integration.txt", "Integration test content").await
            .expect("Failed to write file");
        
        // Test reading the file
        let content = filesystem.read_file("/test/integration.txt").await
        ...
        filesystem.write_file("/test/file1.txt", "Content 1").await
            .expect("Failed to write file1");
        filesystem.write_file("/test/file2.txt", "Content 2").await
    ...
    async fn test_transport_closes_properly() {
        ...
        client_transport.close().await.expect("Failed to close client transport");
        server_transport.close().await.expect("Failed to close server transport");
    }
}
...
mod tool_tests {
    ...
    async fn test_mock_tool_integration() {
        ...
        let result = mock_write_file_tool(&filesystem, "/tools/test.txt", "Tool content").await
            .expect("Tool should succeed");
        
        assert_eq!(result, "Successfully wrote 12 bytes to /tools/test.txt");
        
        // Verify the file was actually written
        let content = filesystem.read_file("/tools/test.txt").await
