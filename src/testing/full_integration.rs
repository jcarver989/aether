...
use rmcp::{
    ...
    tool, tool_handler, tool_router, serve_server, serve_client,
    schemars::JsonSchema,
    service::{RunningService, ServerInitializeError, ClientInitializeError},
...
mod tests {
    ...
    async fn test_real_mcp_server_client_integration() {
        ...
        let call_result = client.call_tool(CallToolRequestParam {
        ...
        let file_content = filesystem.read_file("/test/hello.txt").await
        ...
        }).await.expect("Failed to call write_file tool second time");
        ...
        let second_content = filesystem.read_file("/test/second.txt").await
